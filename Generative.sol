...
contract GENERATIVE is ERC1155, EIP2981, Ownable  {
    using Strings for uint256;

    string public name = "GENERATIVE";
    ...
    constructor(string memory _initNotRevealedUri, string memory _initBaseURI, address _royaltyRecipient, uint256 _royaltyAmount) EIP2981(_royaltyRecipient, _royaltyAmount) ERC1155("") Ownable() {
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(currentSupply + _mintAmount <= maxTotalSupply, "max NFT limit exceeded");
    ...
    if (msg.sender != owner()) {
        ...
        if(preSale){
        require(isPreSaleListed(msg.sender), "user is not presalelisted");
        uint256 ownerMintedCount = addressMintedBalance[msg.sender];
    ...
    for (uint256 i = 1; i <= _mintAmount; i++) {
      ...
      _mint(msg.sender, currentSupply + i, 1, "");
