...
// none ipfs://QmSxUBC2mqbtkpaQG9Agd9tFNpG2RdC44FAN4EqiP9CYj9/
// resting ipfs://Qmd1zf4hSEAYsNrNHassk3eY7e6vnvFc1aGUTJaT2FhV77/
//relaxing ipfs://Qma3Y4mjMdXEkWrC2hJzWLEmSuC6icHRFJz9cMd3yZe8iL/
// awakening ipfs://QmbYa8nnyvKudpuScAZoqzUkjVgipMZCh7ACR3Qk94edVq/
// integrating ipfs://QmYibwP9VpgYRg2H7FY6QcFDpmdWyhLjNy1XoUhgendYph/
// vibrating ipfs://QmQsWzE1bpJ4397S7aDjX92AYKUyvAjyoKeWm4kjiKWmgU/

contract GENERATIVE is ERC721A, EIP2981, Ownable {
   using Strings for uint256;
    ...
    uint256 public stage1Cost = 0.015 ether;
    uint256 public stage2Cost = 0.025 ether;
    uint256 public stage3Cost = 0.0333 ether;
    mapping(address => bool) stage1Minted;
    mapping(address => bool) stage2Minted;
    mapping(address => bool) stage3Minted;
    bool public paused;
    ...
    mapping(uint256 => uint256) public totalNestings;
    bytes32 private stage1RootHash;
    bytes32 private stage2RootHash;
    uint256 private stage1StartTime;
    uint256 private stage2StartTime;
    ...
    constructor(string memory _initNotRevealedUri, bytes32 _stage1RootHash,
        uint256[] memory _nestPeriods, string[] memory _nestURI,
        ...
        nestPeriods = _nestPeriods;
        nestURI = _nestURI;
        require(nestURI.length == nestPeriods.length, "Lengths should be same");
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory)
  {
    ...
    (uint256 currentNest,,) = getNestInfo(tokenId);
    string memory currentBaseURI;
    uint256 periodCount;
    for(uint256 i = 0; i < nestPeriods.length; i++){
        if(currentNest >= nestPeriods[i]){
            periodCount = i;
        }
    }
    currentBaseURI = nestURI[periodCount];
  ...
  function mint(bytes32[] calldata proof) external payable {
    ...
    if (msg.sender != owner()) {
        if(stage1) {
            ...
            require(verifyStage1(proof), "User is not whitelisted!");
            require(!stage1Minted[msg.sender], "Already minted in stage1");
            require(msg.value >= stage1Cost * _mintAmount, "Not enough ETH sent: check whitelist price.");
            stage1Minted[msg.sender] = true;
        } 
        if(stage2){
            ...
            require(msg.value >= stage2Cost * _mintAmount, "Not enough ETH sent: check presale price.");
            stage2Minted[msg.sender] = true;
        }
        if(stage3){
            require(msg.value >= stage3Cost * _mintAmount, "Not enough ETH sent: check public price.");
    ...
    function verifyStage1(bytes32[] memory proof) internal view returns (bool)
  {
    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    return MerkleProof.verify(proof, stage1RootHash, leaf);
  }

  function verifyStage2(bytes32[] memory proof) internal view returns (bool)
  {
    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
  ...
  function withdraw() external payable onlyOwner {
    ...
    function getNestInfo(uint256 tokenId) public view returns (uint256, uint256, bool) 
	{
        require(_exists(tokenId), "Non-existent token");
        bool nesting = _birthdays[tokenId] != 0;
        uint256 currentNest = block.timestamp - _birthdays[tokenId];
        uint256 totalNest = totalNestings[tokenId];
        if(nesting){
            return (currentNest / 1 minutes, (currentNest + totalNest) / 1 minutes, true);
        } return (0, totalNest / 1 minutes, false);
	}
    ...
    function unNestToken(uint256 tokenId) external{
        require(ownerOf(tokenId) == _msgSender(), "You do not own this token.");
        require(_birthdays[tokenId] != 0, "The token wasn't nesting");
        totalNestings[tokenId] += block.timestamp - _birthdays[tokenId];
...
// reveal ipfs://QmcbziCVTb8QsrRyieACWJj5uZwv1hyeXdqVqas4BKyxcv
//stage 1 root 0xb00fabd8a02fbe1e5d188013b233a117c35eba8aa90e4a4c76015a02b1cacb1f
//stage 2 root 0xf29230f9b070aea4fbe0a0afc8b3d1a66178a6ade8f9859a9ef17c18fc6e9738
// ["ipfs://QmPZMbszuXe8PTysAdpN3pmxQPxh7JVDCZU7EZFfue43vE/","ipfs://QmXzn2bCeeKwhmonnZtHVWSavEGs8ywM5MrpA3Cpf7UnTB/","ipfs://QmQnxLYfrAic7qDWiragFqiCgB3i5hVuTLAv3VJokwsN3q/","ipfs://QmbAQYEc5qPJe5mouGVCrgU8sePQUtYqSnN4iGZyfLSs1j/","ipfs://QmSp9eB7oNNeuHKvkh8uMMpARjbp9PA5JiTjyTn1gYhoAS/","ipfs://QmSQap1g8dtbPpPdjsJNyHne4CJx1xj2ZzBr9ddoCCjdtH/"]